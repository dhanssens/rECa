...
def ECa_QP_curve_plot(sensor, precision=.001, noise=0, max_ECa=4, min_ECa=0.0001, init_MSa=.0, alt_MSa=100e-5, plot=False):
    """
        Calculates the ECa-QP and -IP curve.
        ...
        precision: float, optional
            Required ECa precision (S/m)

        noise: float, optional
            Instrument noise (ppm)

        max_ECa: float, optional
            Maximum sampled homogeneous EC value (S/m)

        min_ECa: float, optional
            Minimum sampled homogeneous EC value (S/m)

        init_MSa: float, optional
            Homogeneous MS value, 0 by default

        plot: bool, optional
            Plot ECa-QP and -IP curve
    ...
    # Initialize model characteristics
    sus = np.array([init_MSa])
    perm = np.array([epsilon_0])
    thick = np.array([0])

    # Initial EC range (S/m)
    number_of_EC_samples = int(np.round((max_ECa - min_ECa) * 1000))
    if number_of_EC_samples > 20000: raise ValueError('max ECa is too high, make sure it is in S/m')  # Prevent overflow
    ECa = np.linspace(min_ECa, max_ECa, number_of_EC_samples)
    ...
    if noise != 0:
        ...
        # Calculate slope for required precision
        QP_diff = np.abs(np.diff(QP[::precision])) - (np.abs(QP[::precision] - QP_alt[::precision]))[:-1]

        # Account for noise and single valued non-robust locations
        noise_mask = QP_diff < noise
        mask = noise_mask #& (np.diff(np.append(noise_mask, [True])) == False)

        # Grab non-robust ECa and QP values
        non_robust_ECa = ECa[:-precision:precision][mask]
        non_robust_QP = QP[:-precision:precision][mask]
        ...
        if plot:
            ...
            plt.plot(ECa, QP, 'k')
            plt.plot(ECa, IP, 'k--')
            plt.plot(non_robust_ECa, non_robust_QP, 'r.', markersize=2)
            plt.xlabel('ECa (S/m)')
            plt.ylabel('H (ppm)')
            plt.legend(['QP', 'IP', 'non-robust'], frameon=False)
            plt.title('ECa-QP and -IP curve')
        ...
        if plot:
            ...
            plt.xlabel('ECa (S/m)')
            plt.ylabel('H (ppm)')
            plt.legend(['QP', 'IP'], frameon=False)
            plt.title('ECa-QP and -IP curve')

    # Return ECa, QP, IP, non_robust_ECa
    return ECa, QP, IP, non_robust_ECa, non_robust_QP